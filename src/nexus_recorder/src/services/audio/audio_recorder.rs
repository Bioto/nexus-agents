...
pub struct AudioStream {
    ...
    #[cfg(target_os = "linux")]
    _parecord_process:
        Option<Arc<std::sync::Mutex<Option<(std::process::Child, std::path::PathBuf)>>>>,
    #[cfg(not(target_os = "linux"))]
...
impl AudioStream {
    ...
    pub fn play(&mut self) -> Result<()> {
        ...
        self.stream
            .start()
            .map_err(|e| RecorderError::Audio(format!("Failed to start audio stream: {}", e)))
    }
    ...
    pub fn pause(&mut self) -> Result<()> {
        ...
        self.stream
            .stop()
            .map_err(|e| RecorderError::Audio(format!("Failed to stop audio stream: {}", e)))
    }
}
...
impl AudioRecorder {
    ...
    /// # Errors
    ///
    /// Returns [`RecorderError::Audio`] if PortAudio initialization fails.
    #[must_use = "AudioRecorder creation may fail and the Result should be handled"]
    pub fn new() -> Result<Self> {
        let pa = pa::PortAudio::new()
            .map_err(|e| RecorderError::Audio(format!("Failed to initialize PortAudio: {}", e)))?;
    ...
    pub fn list_input_devices(&self) -> Result<Vec<DeviceInfo>> {
        let num_devices = self
            .pa
            .device_count()
            .map_err(|e| RecorderError::Audio(format!("Failed to get device count: {}", e)))?;

        let default_input = self.pa.default_input_device().ok(); // PortAudio returns Result, we want Option for comparison
    ...
    pub fn list_output_devices(&self) -> Result<Vec<DeviceInfo>> {
        ...
        let default_output = self.pa.default_output_device().ok(); // PortAudio returns Result, we want Option for comparison
    ...
    pub fn list_monitor_devices(&self) -> Result<Vec<DeviceInfo>> {
        ...
        let pulse_monitor_patterns = ["pulse", "Monitor of PulseAudio", "Monitor of PipeWire"];
            ...
            if let Ok(device_info) = self.pa.device_info(idx) {
                if device_info.max_input_channels > 0 {
                    ...
                    if let Some(ref requested_card) = requested_card_name {
                        ...
                        if device_name
                            .to_lowercase()
                            .contains(&requested_card.to_lowercase())
                        {
                            return Ok(Some(idx));
                        }
                    }
    ...
    pub fn default_input_device_index(&self) -> Result<pa::DeviceIndex> {
        self.pa
            ...
            .map_err(|e| RecorderError::Audio(format!("No default input device available: {}", e)))
    }
    ...
    pub fn default_output_monitor_name(&self) -> Result<String> {
        #[cfg(target_os = "linux")]
        {
            ...
            if let Ok(pulse_monitor_name) = Self::get_pulseaudio_monitor_source() {
                log::info!(
                    "Using existing PulseAudio monitor source: {}",
                    pulse_monitor_name
                );
    ...
    #[cfg(target_os = "linux")]
    pub fn create_loopback_sink(
        sink_name: Option<&str>,
    ) -> std::result::Result<(String, Vec<u32>, String), String> {
        ...
        if sinks.lines().any(|line| line.contains(sink_name)) {
            ...
            if let Ok(module_id) = module_id_str.parse::<u32>() {
                log::info!(
                    "Created loopback sink '{}' with module ID: {}",
                    sink_name,
                    module_id
                );
        ...
        let default_sink = String::from_utf8_lossy(&default_sink_output.stdout)
            .trim()
            .to_string();
        info!("📺 Default output sink detected: {}", default_sink);
        ...
        if !loopback_exists {
            ...
            info!(
                "📺 Creating combine-sink to route audio to both {} and {}",
            ...
            if let Ok(module_id) = module_id_str.parse::<u32>() {
                info!(
                    "✅ Combine-sink module created (ID: {}) - routing audio to both {} and {}",
                    module_id, default_sink, sink_name
                );
                module_ids.push(module_id);
                ...
                if let Err(e) = Self::set_default_sink(&format!("{}_combined", sink_name)) {
                    warn!("⚠️  Could not set combine-sink as default: {}", e);
                    warn!("   Audio may not route correctly");
                } else {
                    info!(
                        "✅ Set combine-sink as default output - all audio will route through it"
                    );
                }
            }
        } else {
            info!("ℹ️  Combine-sink already exists");
        }
    ...
    pub fn set_default_source(source_name: &str) -> std::result::Result<String, String> {
        ...
        log::info!(
            "Set default source to: {} (previous: {})",
            source_name,
            current_source
        );
        Ok(current_source)
    ...
    fn get_pulseaudio_monitor_source() -> std::result::Result<String, String> {
        ...
        let sink_name = String::from_utf8_lossy(&sink_output.stdout)
            .trim()
            .to_string();
        ...
        if sources.lines().any(|line| line.contains(&monitor_name)) {
            ...
            Err(format!(
                "Monitor source {} not found in PulseAudio",
                monitor_name
            ))
        }
    }
        ...
        if let Some(ref device_name) = config.device_name {
            ...
            if let Ok(device_idx) = device_name.parse::<u32>() {
                if device_idx < num_devices {
                    ...
                    if let Ok(device_info) = self.pa.device_info(idx) {
                        if device_info.max_input_channels > 0 {
                            info!("✅ Using device index {}: {}", device_idx, device_info.name);
                            return Ok(idx);
                        }
                    }
                }
                ...
                if let Ok(device_info) = self.pa.device_info(idx) {
                    if device_info.max_input_channels > 0 {
                        if device_info.name.eq_ignore_ascii_case(device_name)
                            ...
                            || device_name
                                .to_lowercase()
                                .contains(&device_info.name.to_lowercase())
                        {
                            info!("✅ Found device '{}' at index {}", device_info.name, i);
                            return Ok(idx);
                        }
                    }
                }
            ...
            warn!(
                "⚠️  Device '{}' not found, falling back to default",
                device_name
            );
        }
                ...
                if let Ok(device_info) = self.pa.device_info(idx) {
                    if device_info.max_input_channels > 0 {
                        ...
                        Err(RecorderError::Audio(
                            "Default device has no input channels".to_string(),
                        ))
                    }
                } else {
                    Err(RecorderError::Audio(
                        "Failed to get default device info".to_string(),
                    ))
                }
            }
            Err(e) => Err(RecorderError::Audio(format!(
    ...
    fn get_device_stream_params(
        ...
        let device_info = self
            ...
            .map_err(|e| RecorderError::Audio(format!("Failed to get device info: {}", e)))?;
    ...
    /// # Errors
    ///
    /// Returns [`RecorderError::Audio`] if audio device cannot be opened or recording fails.
    ...
    pub fn record_to_file(&self, config: RecordingConfig, output_path: &Path) -> Result<()> {
        ...
        let (mut stream, rx, actual_sample_rate, actual_channels) =
            self.stream_audio_chunks(config.clone())?;
        ...
        let mut wav_writer = WavWriter::new(writer, spec)
            .map_err(|e| RecorderError::Audio(format!("Failed to create WAV writer: {}", e)))?;
        ...
        let wav_writer = write_handle
            .join()
            .map_err(|_| RecorderError::Audio("Writer thread panicked".to_string()))?;
        wav_writer
            .finalize()
            .map_err(|e| RecorderError::Audio(format!("Failed to finalize WAV file: {}", e)))?;
    ...
    pub fn stream_audio_chunks(
        ...
        let stream_settings = self
            .pa
            .default_input_stream_settings(channels, sample_rate, pa::FRAMES_PER_BUFFER_UNSPECIFIED)
            .map_err(|e| RecorderError::Audio(format!("Failed to get stream settings: {}", e)))?;
        ...
        let stream = self
            ...
            .map_err(|e| RecorderError::Audio(format!("Failed to open stream: {}", e)))?;
    ...
    fn stream_audio_chunks_pulseaudio(
        ...
        let device_name = config.device_name.as_ref().ok_or_else(|| {
            RecorderError::Audio("Device name required for PulseAudio monitor".to_string())
        })?;
        ...
        info!(
            "📺 Using parecord for PulseAudio monitor source: {}",
            device_name
        );
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if let Ok(output) = list_output {
                ...
                if !sources.contains(device_name) {
                    warn!(
                        "⚠️  WARNING: Monitor source '{}' not found in PulseAudio sources!",
                        device_name
                    );
                    ...
                    info!("✅ Verified monitor source '{}' exists", device_name);
                }
            }
        }
        ...
        info!(
            "📺 Using temporary file for parecord: {}",
            temp_file.display()
        );
        ...
        let mut parecord = Command::new("parecord")
            ...
            .stderr(Stdio::piped())
            .spawn()
            .map_err(|e| RecorderError::Audio(format!("Failed to spawn parecord: {}. Is PulseAudio utils installed? (package: pulseaudio-utils)", e)))?;

        let mut stderr = parecord
            .stderr
            .take()
            .ok_or_else(|| RecorderError::Audio("Failed to get parecord stderr".to_string()))?;
            ...
            loop {
                match stderr.read(&mut stderr_buf) {
                    ...
                    Ok(n) => {
                        ...
                        warn!("⚠️  parecord stderr: {}", error_msg.trim());
                    }
                    Err(_) => break,
                }
            }
        ...
        let read_handle = std::thread::spawn(move || -> Result<()> {
            info!(
                "📺 parecord reader thread started, reading from: {}",
            ...
            let mut file = std::fs::OpenOptions::new()
                ...
                .map_err(|e| {
                    RecorderError::Audio(format!("Failed to open parecord output file: {}", e))
                })?;
                ...
                match file.read(&mut buffer) {
                    ...
                    Ok(n) => {
                        ...
                        if !samples.is_empty() {
                            ...
                            if tx_clone.send(samples).is_err() {
                                info!("📺 parecord receiver dropped, stopping");
                    ...
                    Err(e) => {
                        error!("❌ Error reading from parecord file: {}", e);
                        break;
                    }
                }
            }
            info!(
                "📺 parecord reader thread finished (read {} bytes, {} samples)",
        ...
        let process_handle = Arc::new(std::sync::Mutex::new(Some((
            parecord,
            temp_file_clone_for_cleanup,
        ))));
        ...
        let dummy_settings = self
            .pa
            .default_input_stream_settings(1, 44100.0, pa::FRAMES_PER_BUFFER_UNSPECIFIED)
            .map_err(|e| {
                RecorderError::Audio(format!("Failed to create dummy stream settings: {}", e))
            })?;

        let dummy_stream = self
            .pa
            .open_non_blocking_stream(
                dummy_settings,
                move |_| pa::Continue, // Dummy callback
            )
            .map_err(|e| RecorderError::Audio(format!("Failed to create dummy stream: {}", e)))?;
...
mod tests {
    ...
    #[test]
    #[ignore] // Requires audio hardware - may segfault in headless/CI environments
