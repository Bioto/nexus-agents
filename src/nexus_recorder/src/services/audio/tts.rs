...
impl TextToSpeech {
    ...
    pub fn with_config(config: TtsConfig) -> Result<Self> {
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| RecorderError::Api(format!("Failed to create HTTP client: {}", e)))?;
        Ok(Self {
            http_client: client,
    ...
    async fn synthesize_http(&self, text: &str, config: &TtsConfig) -> Result<Vec<u8>> {
        ...
        if !status.is_success() {
            ...
            log::error!("{}", err_msg);
            return Err(RecorderError::Api(err_msg));
        }
    ...
    async fn synthesize_websocket(&self, text: &str, config: &TtsConfig) -> Result<Vec<u8>> {
        ...
        let mut url = Url::parse(endpoint)
            .map_err(|e| RecorderError::Api(format!("Invalid WebSocket URL: {}", e)))?;
        ...
        request.headers_mut().insert(
            ...
            HeaderValue::from_str(&api_key)
                .map_err(|e| RecorderError::Api(format!("Failed to create header value: {}", e)))?,
        );
        ...
        let send_handle = tokio::spawn(async move {
            ...
            for word in text_to_send.split_whitespace() {
                ...
                write
                    ...
                    .await
                    .map_err(|e| RecorderError::Api(format!("Failed to send text: {}", e)))?;
            }
            ...
            eos_msg
                ...
                .map_err(|e| RecorderError::Api(format!("Failed to serialize EOS: {}", e)))?;

            write
                .send(Message::Binary(buf))
                .await
                .map_err(|e| RecorderError::Api(format!("Failed to send EOS: {}", e)))?;
        ...
        while let Some(msg) = read.next().await {
            match msg {
                ...
                Err(e) => {
                    ...
                    drop(audio_tx);
                    return Err(RecorderError::Api(format!("WebSocket error: {}", e)));
                }
            }
        }
    ...
    async fn play_pcm_samples(
        ...
        let stream = device
            ...
            .map_err(|e| RecorderError::Audio(format!("Failed to build output stream: {}", e)))?;

        // Play the stream
        stream
            .play()
            .map_err(|e| RecorderError::Audio(format!("Failed to play audio stream: {}", e)))?;
        ...
        stream
            .pause()
            .map_err(|e| RecorderError::Audio(format!("Failed to pause audio stream: {}", e)))?;
    ...
    pub async fn list_voices(&self) -> Result<Vec<VoiceInfo>> {
        ...
        let voices: Vec<serde_json::Value> = response
            .json()
            .await
            .map_err(|e| RecorderError::Api(format!("Failed to parse voices response: {}", e)))?;
        Ok(voices
            .into_iter()
            .map(|v| VoiceInfo {
