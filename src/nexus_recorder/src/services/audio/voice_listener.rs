...
impl VoiceListener {
    ...
    /// # Errors
    ///
    /// Returns [`RecorderError::Configuration`] if the Whisper model path is invalid.
    /// Returns [`RecorderError::Audio`] if audio recorder initialization fails.
    ...
    /// # Errors
    ///
    /// Returns [`RecorderError::Other`] if the listener is already running.
    /// Returns [`RecorderError::Audio`] if audio stream initialization fails.
        ...
        F: FnMut(VoiceMetrics),
    {
        ...
        stream
            .play()
            .map_err(|e| RecorderError::Audio(format!("Failed to start stream: {}", e)))?;
        ...
        stream
            .pause()
            .map_err(|e| RecorderError::Audio(format!("Failed to stop stream: {}", e)))?;
