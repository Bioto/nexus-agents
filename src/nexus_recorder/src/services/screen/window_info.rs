...
impl WindowInfoService {
    ...
    fn get_window_geometry_wmctrl(window_id: &str) -> Result<WindowGeometry> {
        let output = Command::new("wmctrl")
            ...
            .arg("-l")
            .output()
            .map_err(|e| RecorderError::Configuration(format!("wmctrl -i -G -l failed: {}", e)))?;

        if !output.status.success() {
            return Err(RecorderError::Other("wmctrl -i -G -l failed".to_string()));
        }
            ...
            if let Some(id) = parts.get(0) {
                if *id == window_id {
                    let x = parts
                        ...
                        .ok_or_else(|| RecorderError::Other("Invalid X coord".to_string()))?;
                    let y = parts
                        .get(3)
                        .and_then(|s| s.parse::<i32>().ok())
                        .ok_or_else(|| RecorderError::Other("Invalid Y coord".to_string()))?;
                    let width = parts
                        .get(4)
                        .and_then(|s| s.parse::<u32>().ok())
                        .ok_or_else(|| RecorderError::Other("Invalid Width".to_string()))?;
                    let height = parts
                        .get(5)
                        .and_then(|s| s.parse::<u32>().ok())
                        .ok_or_else(|| RecorderError::Other("Invalid Height".to_string()))?;
    ...
    fn get_window_pid_linux(window_id: &str) -> Result<u32> {
        ...
        let output = Command::new("xprop")
            ...
            .arg("_NET_WM_PID")
            .output()
            .map_err(|e| RecorderError::Configuration(format!("xprop failed: {}", e)))?;
        ...
        Err(RecorderError::WindowPidLookup {
            window_id: window_id.to_string(),
        })
    }
    ...
    fn list_windows_macos() -> Result<Vec<WindowInfo>> {
        ...
        let output = Command::new("osascript")
            ...
            .output()
            .map_err(|e| {
                RecorderError::Configuration(format!("Failed to run osascript: {}. Make sure you have granted Terminal/your app accessibility permissions.", e))
            })?;
