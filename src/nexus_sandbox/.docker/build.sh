#!/bin/bash
# Build script for nexus_sandbox Docker image
...
#   IMAGE_NAME      - Docker image name (default: nexus_sandbox)
#   IMAGE_TAG       - Docker image tag (default: latest)
#   DOCKERFILE_PATH - Path to Dockerfile (default: src/nexus_sandbox/.docker/Dockerfile)
#   BUILD_CONTEXT   - Build context directory (default: .)

set -e

# Default values (matching DockerConfig defaults)
IMAGE_NAME="${IMAGE_NAME:-nexus_sandbox}"
IMAGE_TAG="${IMAGE_TAG:-latest}"
DOCKERFILE_PATH="${DOCKERFILE_PATH:-src/nexus_sandbox/.docker/Dockerfile}"
...
# Script is in src/nexus_sandbox/.docker, so workspace root is ../../.. from script
