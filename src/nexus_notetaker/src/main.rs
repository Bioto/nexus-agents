use chrono;
use clap::Parser;
use nexus_notetaker::{Cli, Commands};
use simplelog::{
...
fn init_logging() -> String {
    LOG_INIT
        .get_or_init(|| {
            ...
            let log_file = log_dir.join(format!("nexus_notetaker_{}.log", timestamp));
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Commands::Process(args) => nexus_notetaker::run_process(args).await?,
    }
