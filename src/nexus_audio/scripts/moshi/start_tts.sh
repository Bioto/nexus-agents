...
# Install Python dependencies required by moshi-server
# Use uv sync if lock file exists, otherwise install from pyproject.toml
if [ -f uv.lock ]; then
    uv sync
else
    uv pip install -e .
...
# Run moshi-server directly (it's installed via cargo install)
# LD_LIBRARY_PATH must be set for moshi-server to find Python libraries
moshi-server worker --config "./configs/tts.toml" --port 8089