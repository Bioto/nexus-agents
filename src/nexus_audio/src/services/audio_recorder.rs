...
impl AudioRecorder {
    ...
    pub fn default_output_monitor_name(&self) -> Result<String> {
        ...
        let default_output_info = self.pa.device_info(default_output_idx).map_err(|e| {
            VoiceError::Audio(format!("Failed to get default output device info: {}", e))
        })?;
    ...
    fn get_input_device_index(&self, config: &RecordingConfig) -> Result<pa::DeviceIndex> {
        ...
        if let Some(ref device_name) = config.device_name {
            ...
            if let Ok(device_idx) = device_name.parse::<u32>() {
                if device_idx < num_devices {
                    ...
                    if let Ok(device_info) = self.pa.device_info(idx) {
                        if device_info.max_input_channels > 0 {
                            eprintln!("✅ Using device index {}: {}", device_idx, device_info.name);
                            return Ok(idx);
                        }
                    }
                }
        ...
        match self.pa.default_input_device() {
            ...
            Err(e) => Err(VoiceError::Audio(format!(
    ...
    fn stream_audio_chunks_pulseaudio(
        ...
        let device_name = config.device_name.as_ref().ok_or_else(|| {
            VoiceError::Audio("Device name required for PulseAudio monitor".to_string())
        })?;
        ...
        let read_handle = std::thread::spawn(move || -> Result<()> {
            ...
            let mut file = std::fs::OpenOptions::new()
                ...
                .map_err(|e| {
                    VoiceError::Audio(format!("Failed to open parecord output file: {}", e))
                })?;
        ...
        let dummy_settings = self
            ...
            .map_err(|e| {
                VoiceError::Audio(format!("Failed to create dummy stream settings: {}", e))
            })?;
