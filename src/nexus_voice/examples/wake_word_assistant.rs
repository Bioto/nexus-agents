use nexus_voice::services::{
    TranscriptionHandler, TranscriptionResult, VoiceListener, VoiceListenerConfig,
};
use std::sync::mpsc;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    std::thread::spawn(move || {
        while let Ok(result) = rx.recv() {
            println!(
                "Received: {} (duration: {:.1}s, time: {:?})",
                result.text, result.duration_seconds, result.timestamp
            );
