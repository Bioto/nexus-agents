...
1. Create the certificate file:
   ...
   cat > .docker/nutrition-mcp/ssl/cert.pem << 'EOF'
   -----BEGIN CERTIFICATE-----
...
2. Add the private key:
   ...
   cat > .docker/nutrition-mcp/ssl/key.pem << 'EOF'
   -----BEGIN PRIVATE KEY-----
...
3. Set proper permissions:
   ```bash
   chmod 644 .docker/nutrition-mcp/ssl/cert.pem
   chmod 600 .docker/nutrition-mcp/ssl/key.pem
   ```
...
```bash
docker compose -f .docker/nutrition-mcp/docker-compose.yaml restart nginx
```

Check the logs to confirm HTTPS is enabled:
```bash
docker compose -f .docker/nutrition-mcp/docker-compose.yaml logs nginx
```
